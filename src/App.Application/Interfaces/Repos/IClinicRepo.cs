...
namespace App.Application.Interfaces.Repos
{
  ...
  {
    public Task<Clinic?> GetByIdAsync(Guid id, CancellationToken token = default); 
    ...
    public Task<bool> Exists(Guid id, CancellationToken token = default); 
    public Task<int> UpdateAsync(Clinic clinic, CancellationToken token = default);
  }
}
